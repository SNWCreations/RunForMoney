    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (!sender.isOp()) { // 防止熊孩子乱来hhhhc
                ...
                sender.sendMessage(ChatColor.GREEN + new PlaceHolderString(LanguageSupport.getTranslation("commands.group.leave.success_count")).replaceArgument("count", realArgs.size() - failed.size()).toString());
                if (!failed.isEmpty()) {
