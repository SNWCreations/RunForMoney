        ...
        if (RunForMoney.getInstance().getGameProcess() != null) {
            ...
            if (args.length == 0) {
                ...
                if (!sender.isOp()) {
                    ...
                    sender.sendMessage(ChatColor.GREEN + new PlaceHolderString(LanguageSupport.getTranslation("commands.team.hunter.success_count")).replaceArgument("count", realArgs.size() - failed.size()).toString());
                    if (!failed.isEmpty()) {
